...
export default function PartCard({ part }: PartCardProps) {
  ...
  const { addPart, isAdding: isAddingToCart, updateQuantity, isUpdating } = useCart();
  ...
  const cartItem = useCartStore(
    useMemo(
      () => (state) => {
        ...
        const item = state.cart.items.find(item => item.partId === part.id);
        if (!item) return null;
  ...
  const primaryImage = part.images?.find((img: PartImage) => img.isPrimary) || part.images?.[0];
  const imageUrl = primaryImage?.cloudinaryUrl || '/placeholder-part.jpg';
        ...
        <div className="flex items-center justify-between pt-4 border-t border-gray-100">
          ...
          {!isInCart ? (
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              disabled={part.stockQuantity === 0 || isAddingToCart}
            ...
            <div className="flex items-center gap-2 bg-blue-50 rounded-lg p-1 border border-blue-200">
              ...
              <motion.button
                whileHover={{ scale: 1.1 }}
                whileTap={{ scale: 0.9 }}
                disabled={isUpdating || currentQuantity >= part.stockQuantity}
                className="p-2 rounded-md bg-white hover:bg-blue-100 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
