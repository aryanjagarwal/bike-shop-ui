...
export default function CheckoutForm() {
  ...
  const { items, getCartTotal, getSelectedItemsTotal, clearCart } = useCartStore();
  ...
  // Calculate selected items for UI display
  const selectedItems = new Set(
    items
      .filter(item => item.product.isSelected)
      .map(item => item.id)
      .filter(Boolean) as string[]
  );
  
  const hasSelectedItems = selectedItems.size > 0;
  const displayTotal = hasSelectedItems ? getSelectedItemsTotal(selectedItems) : getCartTotal();
  const selectedItemsCount = hasSelectedItems ? selectedItems.size : items.length;
                    ...
                    <div className="space-y-3">
                      <div className="flex justify-between items-center">
                        <span className="text-muted-foreground">
                          {hasSelectedItems ? `Selected Items (${selectedItemsCount})` : `Items (${items.length})`}
                        </span>
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        <span className="font-medium text-green-600">
                          {(() => {
                            const total = displayTotal - (appliedDiscount || 0);
                            if (
                              total >=
