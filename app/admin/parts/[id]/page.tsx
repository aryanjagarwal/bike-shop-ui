...
export default function EditPartPage() {
  ...
  const { data: partData, isLoading } = useAdminPart(partId);
  ...
  useEffect(() => {
    if (part) {
      setFormData({
        ...
        installationDifficulty: part.installationDifficulty as keyof typeof InstallationLevel,
    ...
    uploadImages.mutate(
      {
        partId,
        images: newImages,
        isPrimary: !part?.images || part.images.length === 0,
      },
      {
        onSuccess: () => {
  ...
  const handleUpdateStock = async () => {
    if (!stockUpdate.reason) {
      alert("Please provide a reason for stock update");
      return;
    }
    ...
    updateStock.mutate(
      ...
      },
      {
        onSuccess: () => {
          ...
          setStockUpdate({ ...stockUpdate, reason: "" });
        },
        onError: () => {
    ...
    return (
      <div className="max-w-4xl">
        <div className="bg-white rounded-lg shadow-sm p-12 text-center">
          ...
          <p className="mt-4 text-gray-600">Loading part...</p>
        </div>
      </div>
    );
  }
  ...
  const isSubmitting = updatePart.isPending;
          ...
          <div>
            ...
            <div className="bg-gray-50 p-4 rounded-lg">
              <p className="text-sm text-gray-600 mb-4">
                Current Stock: <span className="font-semibold">{part.stockQuantity}</span>
              </p>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
              ...
              <button
                ...
                className="mt-4 px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors disabled:bg-gray-300"
              >
